...
import { getBasePath, getUsernameFromEmail } from '../../../utils/sharedFunctions.js';
...
const createSwitchUserModal = () => {
    ...
    savedUsers.forEach(user => {
        ...
        userContainer.addEventListener('click', () => {
            ...
            //alert(`Switched to user: ${user.userName || user.email}`);
    ...
    addButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (docSnap.exists) {
                ...
                alert(`User ${userData.userName || userData.email} added successfully!`);
                document.body.removeChild(modal);
...
// Update the list of saved users in localStorage
const updateSavedUsers = (userId, userData) => {
    const savedUsers = JSON.parse(localStorage.getItem('savedUsers')) || [];

    // Check if the user already exists in the list
    const existingUserIndex = savedUsers.findIndex(user => user.id === userId);
